...
describe("CheckoutPhase tests", () => {
    ...
    it("checkoutPhase: session cart (without conekta id) / city or national / card", async () => {
    ...
    it("checkoutPhase: session cart (with conekta id) / city or national / card", async () => {
        const session_cart_oid = new ObjectId()
        const session_cart_id = session_cart_oid.toHexString()
        const cartsByUser = dbInstance.collection<CartsByUserMongo>("carts_by_user")
        const itemsByCart = dbInstance.collection<ItemsByCartMongo>("items_by_cart")
    ...
    it("checkoutPhase: user cart (without address id) / city or national / card", async () => {
        ...
        const { addresses, ...userWithoutAddress } = restUser
