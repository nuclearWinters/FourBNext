...
describe("CheckoutPhase tests", () => {
    ...
    it("checkoutPhase: session cart (without conekta id) / store / transfer", async () => {
        ...
        expect(spyCreateCustomer).toHaveBeenCalledWith(
            {
                email,
                name: `${name} ${apellidos}`,
                phone,
    ...
    it("checkoutPhase: session cart (with conekta id) / store / transfer", async () => {
        const session_cart_oid = new ObjectId()
        const session_cart_id = session_cart_oid.toHexString()
        const cartsByUser = dbInstance.collection<CartsByUserMongo>("carts_by_user")
        const users = dbInstance.collection<UserMongo>("users")
    ...
    it("checkoutPhase: user cart / store / transfer", async () => {
        const session_cart_oid = new ObjectId()
        const session_cart_id = session_cart_oid.toHexString()
